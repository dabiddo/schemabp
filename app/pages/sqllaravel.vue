  ...
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4">
      <h1 class="text-3xl font-bold text-center mb-8 text-gray-800">
        SQL to Laravel (Migrations & Models) Converter
      </h1>
        ...
        >
          ...
          <div class="relative group">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 border-l-4 border-orange-500">
              <h3 class="text-sm font-bold uppercase text-gray-500 mb-2">Eloquent Model</h3>
              <pre class="bg-gray-50 p-4 rounded font-mono text-xs overflow-x-auto">{{ output.model }}</pre>
            </div>
          </div>
          ...
          <div class="relative group">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6 border-l-4 border-blue-500">
              <h3 class="text-sm font-bold uppercase text-gray-500 mb-2">Migration</h3>
              <pre class="bg-gray-50 p-4 rounded font-mono text-xs overflow-x-auto">{{ output.migration }}</pre>
            </div>
          </div>
        </div>
      </div>
...
const generateLaravelCode = () => {
  try {
    const generators = getGenerators(sqlInput.value, 'sql')
    laravelOutputs.value = generators.sqlToLaravelCode()
  } catch (e) {
    alert("Parsing failed: " + (e as Error).message)
  }
}
