  ...
  <div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4">
      <h1 class="text-3xl font-bold text-center mb-8 text-gray-800">
        JSON-LD to Drizzle ORM (PostgreSQL) Converter
      </h1>
      ...
      <div v-for="(code, index) in drizzleCode" :key="index" class="mb-8">
        <div class="relative group">
          ...
          <div class="bg-white rounded-lg shadow-sm p-6 border-l-4 border-indigo-500">
            ...
            <pre class="bg-gray-50 p-4 rounded font-mono text-xs overflow-x-auto">{{ code }}</pre>
          </div>
        </div>
      </div>
    </div>
...
const drizzleCode = ref<string[]>([])

const generateDrizzleCode = () => {
  try {
    const generators = getGenerators(jsonLdInput.value, 'json')
    drizzleCode.value = generators.toDrizzleCode()
  } catch (e) {
